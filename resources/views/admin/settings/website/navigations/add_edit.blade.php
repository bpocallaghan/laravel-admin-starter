            ...
            <div class="box box-primary box-solid">
                <div class="box-header with-border">
                    <h3 class="box-title">
                        ...
                        <span>{{ isset($item)? 'Edit the ' . $item->title . ' entry': 'Create a new Navigation' }}</span>
                    </h3>
                </div>
                ...
                <div class="box-body no-padding">
                    ...
                    <fieldset>
                        ...
                        <div class="row">
                            <div class="col col-6">
                                <section class="form-group {{ form_error_class('description', $errors) }}">
                                    <label for="id-parent_id">Parent (navigation parent, under which navigation it will display)</label>
                                    {!! Form::select('parent_id', ([0 => 'Please select a Parent'] + $parents), isset($item)? ($errors && $errors->any()? old('parent_id') : $item->parent_id) : old('parent_id'), ['class' => 'select2 form-control ']) !!}
                            ...
                            <div class="col col-6">
                                <section class="form-group {{ form_error_class('url_parent_id', $errors) }}">
                                    <label for="id-parent_id">Url Parent (parent that will be used to generate the url, same as parent if empty</label>
                                    {!! Form::select('url_parent_id', ([0 => 'Please select a Url Parent'] + $parents), ($errors && $errors->any()? old('url_parent_id') : (isset($item)? $item->url_parent_id : '')), ['class' => 'select2 form-control']) !!}
                        ...
                        <section class="form-group {{ form_error_class('banners', $errors) }}">
                            <label for="banners">Banners</label>
                            {!! Form::select('banners[]', $banners, ($errors && $errors->any()? old('banners') : (isset($item)? $item->banners->pluck('id')->all() : '')), ['class' => 'select2 form-control', 'multiple']) !!}
